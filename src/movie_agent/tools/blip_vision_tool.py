...
from ..utils.hardware import HardwareDetector, DeviceType, get_recommended_dtype
...
class BLIPVisionTool(VisionTool):
    ...
    def __init__(
        ...
        :param device: Device to run on ("cuda", "mps", "cpu", or None for auto-detect)
        :param config: Optional MovieAgentConfig for hardware configuration
        """
        self.model_name = model_name or "Salesforce/blip-image-captioning-base"
        self.model_path = model_path
        self.config = config
        
        # Determine device: explicit device > config > auto-detect
        if device is not None:
            self.device = device
        elif config is not None:
            # Use config-based device selection
            if config.force_cpu:
                self.device = "cpu"
            else:
                device_pref = DeviceType(config.device) if config.device else DeviceType.AUTO
                self.device = HardwareDetector.select_device(device_pref)
        else:
            # Fallback to auto-detect
            self.device = HardwareDetector.select_device(DeviceType.AUTO)
    ...
    def _load_model(self):
        ...
        try:
            # Use hardware-aware dtype selection
            dtype_str = get_recommended_dtype(self.device)
