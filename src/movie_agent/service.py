...
class MovieAgentService:
    ...
    def __init__(self, config: MovieAgentConfig):
        ...
        # Log hardware information if enabled
        if self.config.log_hardware_info:
            hardware_info = HardwareDetector.detect_all()
            HardwareDetector.log_hardware_info(hardware_info)
            logger.info(f"Device configuration: device={config.device}, force_cpu={config.force_cpu}, faiss_gpu={config.faiss_gpu_enabled}")
